...
**Дата последнего обновления: 2026-09-19 (МСК) — v3.4: Mac-сборка интегрирована (BUG-009…016), GitHub-репо восстановлено после web-загрузки, см. CHANGELOG.md**
...
**Этап: v3.4 — Mac-сборка РАБОТАЕТ (на маке пользователя). Репозиторий восстановлен
и свёрнут в чистую историю (web-загрузка «Add files via upload» вычищена force-push'ом).
Следующие шаги: остаток BUG-015 (онлайн-индикатор ~30 с), ЗАМ-001 (раздельные состояния
движка: не запущен / запускается / онлайн / не отвечает / упал), ЗАМ-002 (dedup тостов),
ЗАМ-004 (supervisor: ранний выход, если бинарника нет — не 3 бессмысленных ретрая),
ручной macOS-прогон (NSOpenPanel/меню/DnD/kill-on-exit/App Nap).**
