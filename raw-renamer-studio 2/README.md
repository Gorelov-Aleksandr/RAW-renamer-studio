...
├── tools/make_demo_batch.py  ← генератор демо-партии (fixture)
├── demo_batch/               ← демо: RAW, заявка .xlsx, PIM-выгрузка
...
python3 tools/make_demo_batch.py          # демо-партия (22 RAW + заявка + PIM)
...
python3 src-python/server.py --port 0 --port-file .sidecar/port \
  --data-dir .data --demo-folder demo_batch \
  --zayavka demo_batch/zayavka_17.09.2026.xlsx --watchdog 0

# терминал 2 — фронтенд (Vite проксирует /rpc и /preview на порт из .sidecar/port):
npm run dev                                # http://localhost:5173
```

Откройте http://localhost:5173 → демо-партия подхватится автоматически.
...
- Иконки: `npm run tauri icon src-tauri/icons/icon.png` (базовая иконка уже
  сгенерирована в `tools/make_demo_batch.py`).
...
| `system.info` | версия, порт, движки ШК, путь демо, размер кэша |
...
`demo_batch/shoot_2026-09-16/` — 6 товаров, 22 «RAW» (`.CR2` со встроенным JPEG).
Первый кадр каждого товара — этикетка с **настоящим** EAN-13, который реально
распознаётся OpenCV/ZXing. Заявка `zayavka_17.09.2026.xlsx` собрана из PIM-выгрузки
через тот же `pim_builder`, что использует продакшен.

Проверка сценария в UI: открыть демо-партию → «Переименовать всё» → подтвердить →
`89458028.CR2`, `89458028_y.CR2`, … + L/M в Excel → «Отменить ⌘Z» → исходные имена.
