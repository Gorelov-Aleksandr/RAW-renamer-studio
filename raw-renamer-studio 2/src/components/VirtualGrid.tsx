...
export default function VirtualGrid() {
  ...
  const items = useMemo(() => {
    ...
    if (q) arr = arr.filter((it) => `${it.lm_code ?? ''} ${it.barcode ?? ''}`.toLowerCase().includes(q));
          ...
          <div className="flex flex-col gap-2 mt-5">
            {info?.demo && (
              <button
                ...
                className="h-9 rounded-lg bg-accent-fill hover:bg-[#6A57E2] text-white text-[12.5px] font-bold flex items-center justify-center gap-2"
              >
                <Camera size={15} />
                Открыть демо-партию (6 товаров · 22 RAW)
              </button>
            )}
            <button
  ...
  return (
    <div ref={parentRef} className="flex-1 overflow-y-auto min-h-0">
      <div ref={innerRef} className="px-4 py-2 w-full">
        {items.length === 0 ? (
          <div className="grid place-items-center h-40 text-tx-3 text-[13px]">
