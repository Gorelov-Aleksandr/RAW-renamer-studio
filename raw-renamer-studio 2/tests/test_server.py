...
DEMO = Path(__file__).resolve().parent.parent / 'demo_batch'
DEMO_SHOOT = DEMO / 'shoot_2026-09-16'
...
def _demo_csv_variant(sc, enc, bom, delim, tag) -> dict:
...
class TestSystem:
    ...
    def test_info(self, sc):
        ...
        assert r['demo'] and r['demo']['raw_folder'].endswith('shoot_2026-09-16')
...
class TestSession:
    ...
    def test_open_demo_cv(self, sc):
        info = sc.ok('system.info')
        r = sc.ok('session.open_folder', {
            'folder': info['demo']['raw_folder'], 'mode': 'cv',
            'zayavka': info['demo']['zayavka']})
        assert len(r['items']) == 6
        assert r['total_frames'] == 22
        assert all(i['status'] == 'ok' for i in r['items'])
        assert all(i['barcode'] for i in r['items'])

    def test_open_folder_names_mode(self, sc):
        info = sc.ok('system.info')
        r = sc.ok('session.open_folder', {
            'folder': info['demo']['raw_folder'], 'mode': 'names'})
        # демо-имена _M2A… не содержат ШК → 22 сироты
    ...
    def test_missing_zayavka(self, sc):
        info = sc.ok('system.info')
        e = sc.err('session.open_folder', {
            'folder': info['demo']['raw_folder'], 'mode': 'cv', 'zayavka': '/nope/none.xlsx'})
        assert e['code'] == 4040

    def test_set_zayavka_missing(self, sc):
        sc.err('session.set_zayavka', {'path': '/nope/none.xlsx'})

    def test_get(self, sc):
        info = sc.ok('system.info')
        sc.ok('session.open_folder', {'folder': info['demo']['raw_folder'], 'mode': 'cv'})
        r = sc.ok('session.get')
        assert r['items'] and r['mode'] == 'cv'

    def test_move_frame_bounds(self, sc):
        info = sc.ok('system.info')
        r = sc.ok('session.open_folder', {'folder': info['demo']['raw_folder'], 'mode': 'cv'})
        it = r['items'][0]
    ...
    def test_suffix_validation(self, sc):
        info = sc.ok('system.info')
        r = sc.ok('session.open_folder', {'folder': info['demo']['raw_folder'], 'mode': 'cv'})
        it = r['items'][0]
    ...
    def test_apply_barcode_bad(self, sc):
        info = sc.ok('system.info')
        r = sc.ok('session.open_folder', {'folder': info['demo']['raw_folder'], 'mode': 'names'})
        it = r['items'][0]
    ...
    def test_apply_barcode_with_letters(self, sc):
        info = sc.ok('system.info')
        r = sc.ok('session.open_folder', {'folder': info['demo']['raw_folder'], 'mode': 'names'})
        it = r['items'][1]
        # буквы отфильтруются
...
class TestZayavka:
    ...
    def test_generate_all_encodings(self, sc, enc, bom, delim, tag):
        r = _demo_csv_variant(sc, enc, bom, delim, tag)
    ...
    def test_generate_csv_format(self, sc):
        r = _demo_csv_variant(sc, 'utf-8', False, ';', 'csvfmt') if False else None
...
# --------------------------------------------------------------------------- renamer
class TestRenamer:
    def _open_copy(self, sc, tag):
        """Копируем демо-папку и демо-заявку через /upload (как браузерный выбор папки).

        Важно: заявка — КОПИЯ, иначе renamer.execute (write_results) перезапишет
        демо-файл в git и загрязнит демо-данные.
        """
        first = _upload(sc, next(iter(sorted(DEMO_SHOOT.glob('*.CR2')))), tag)
        for f in sorted(DEMO_SHOOT.glob('*.CR2')):
            if f.name != Path(first['path']).name:
                _upload(sc, f, tag)
        info = sc.ok('system.info')
        zay_up = _upload(sc, Path(info['demo']['zayavka']), tag)
        folder = Path(first['path']).parent
        return sc.ok('session.open_folder', {
            'folder': str(folder), 'mode': 'cv', 'zayavka': zay_up['path']})

    def test_full_cycle(self, sc):
        r = self._open_copy(sc, 'cycle_' + uuid.uuid4().hex[:6])
        assert len(r['items']) == 6
...
# --------------------------------------------------------------------------- preview
class TestPreview:
    def test_preview_valid(self, sc):
        info = sc.ok('system.info')
        r = sc.ok('session.open_folder', {'folder': info['demo']['raw_folder'], 'mode': 'cv'})
        it = r['items'][0]
...
class TestConcurrency:
    ...
    def test_open_folder_during_heartbeats(self, sc):
        info = sc.ok('system.info')
