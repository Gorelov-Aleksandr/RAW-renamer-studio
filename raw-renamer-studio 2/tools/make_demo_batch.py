#!/usr/bin/env python3
"""Генератор демо-партии RAW Renamer Studio.

Создаёт в demo_batch/:
  ...
  * export_pim_demo.csv — выгрузка PIM (UTF-16 LE BOM, табуляция);
  * zayavka_17.09.2026.xlsx — заявка, собранная через pim_builder (догфудинг).

Использование:  python3 tools/make_demo_batch.py [project_root]
"""
...
def load_photo(key: str) -> Image.Image:
    ...
    if p.exists():
        ...
        # синтетический фолбэк, если демо-фото не сгенерированы
...
def make_icon() -> None:
    ...
    im = Image.new('RGBA', (512, 512), (13, 14, 18, 255))
    d = ImageDraw.Draw(im)
    d.rounded_rectangle([16, 16, 496, 496], radius=112, fill=(110, 92, 231))
    d.rounded_rectangle([16, 16, 496, 250], radius=112, fill=(141, 126, 245))
    d.text((200, 128), 'R', fill=(13, 14, 18), font=_font(240))
    im.save(icon_dir / 'icon.png')
...
def main() -> None:
    ...
    print(f'OK: RAW-файлов {made}, товаров в PIM {len(rows)}, '
          f'заявка {out_x.name}, иконка src-tauri/icons/icon.png')
