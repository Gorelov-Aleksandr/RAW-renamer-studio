...
**Дата последнего обновления: 2026-09-19 (МСК) — v3.5: демо убрано полностью, поиск ⌘F, прогресс-индикатор запуска, сайдбар разворачивается; папка переименована, main = актуален. См. CHANGELOG.md и AI_MASTER.md**
...
**Этап: v3.5 — «полный инструмент» (демо убрано полностью), поиск по коду/ШК/названию,
реальный прогресс-индикатор запуска, сайдбар разворачивается. Mac-сборка работает
(на маке пользователя). Репозиторий чистый: папка `raw-renamer-studio` (без «2»),
main = актуальная (PR слит), arena — рабочая ветка сессии.
