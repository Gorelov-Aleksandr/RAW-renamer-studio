...
**Дата последнего обновления: 2026-09-19 (МСК) — v3.5.1: догружены 2 legacy-правки (кириллическая «у», редкие RAW-форматы; 82/82 тестов ok, main актуален). См. CHANGELOG.md и AI_MASTER.md**
...
**Этап: v3.5.1 — догружены правки (кириллическая «у» как этикетка в names-mode,
RAW_EXTS += .crw .srf .sr2 .3fr .fff .raw). Сборка TypeScript/Vite без ошибок,
pytest 82/82 passed. Ветка main обновлена и актуальна.
Следующие шаги: (1) пользователь пересобирает на маке `npm run tauri build`
(Rust-изменения v3.4.1/v3.5 не проверялись компиляцией в песочнице — нет cargo);
(2) ручной macOS-прогон (NSOpenPanel/меню/DnD/kill-on-exit/App Nap) + посмотреть
спиннер/прогресс на старте; (3) при желании — Windows (пока не браться).**

СДЕЛАНО v3.5.1 (2026-09-19):
- session.py: кириллическая «у» (опечатка русской раскладки в имени файла)
  распознаётся как этикетка (`hint in ('y', 'у')`), в план всегда пишется 'y';
- raw_engine.py: RAW_EXTS дополнен редкими/старыми форматами:
  .crw (Canon до 2003), .srf, .sr2 (Sony), .3fr (Hasselblad), .fff (старый Fuji),
  .raw (общее);
- тесты: +2 автотеста (test_cyrillic_u_label_suffix, test_rare_raw_extensions),
  всего 82/82 passed.

СДЕЛАНО v3.5: папка переименована (без «2»); УБРАНО ВСЁ ДЕМО (кнопки, openDemo,
автооткрытие, поле 'demo' из system.info, --demo-folder, demo_batch/ из репо —
тесты генерируют партию в tmp, 80/80); прогресс запуска: SIDECAR_BOOT маркеры →
sidecar://progress → спиннер «Запуск движка…» + 4 сегмента в шапке; ПОИСК ⌘F
(код LM/ШК/название, все артикулы, «Сбросить поиск»); сайдбар: рельс + кнопка
разворота после сворачивания.
...
ХРОНИКА:
V3.5. **«ПОЛНЫЙ ИНСТРУМЕНТ» (2026-09-19) — СДЕЛАНО:**
   - По запросу пользователя: «никакие демо версии, демо режимы, демо фото,
     демо заявки мне не нужны» → удалено ВСЁ демо из приложения И из репо:
     демо-кнопки (Sidebar, VirtualGrid, ZayavkaModal, палитра ⌘K), openDemo,
     демо-автооткрытие при старте, поле 'demo' из system.info, --demo-folder,
     demo_batch/ + make_demo_batch.py (git rm). Тесты 80/80: конфест генерирует
     тестовую партию в tmp через tools/make_test_batch.py (параметризован,
     фото — синтетика, ШК — настоящие EAN-13), export_pim_demo.csv → export_pim.csv
   - Папка проекта: `raw-renamer-studio 2` → `raw-renamer-studio` (git mv;
     «двойка» мешала, мог быть конфликт)
   - Прогресс-индикатор запуска: sidecar печатает SIDECAR_BOOT (imports/listen),
     Rust → sidecar://progress (0..3), UI — спиннер + «Запуск движка…» + 4 сегмента
     (реальный прогресс Python-фазы; распаковка PyInstaller честно не показана)
   - Поиск (⌘F) по коду LM / ШК / названию среди всех артикулов (поля независимо),
     пустой результат → «Ничего не найдено…» + «Сбросить поиск»; ⌘F — фокус поля
   - Сайдбар: после «Свернуть» остался рельс 34px с кнопкой разворота
     (PanelLeftOpen) — раньше панель исчезала безвозвратно
   - GitHub: PR #1 слит в main (fast-forward merge) — main актуален;
     пользователь выгружает архив с main
   - Проверено: tsc 0, vite build OK, pytest 80/80, boot-маркеры в логе,
     system.info без 'demo', dev-прокси heartbeat OK
   - ВНИМАНИЕ: Rust (sidecar.rs, lib.rs) не компилировался в песочнице (нет cargo) —
