...
def main() -> None:
    ...
    def _boot_stage(stage: str) -> None:
        # v3.5: маркер стадии запуска — Rust пересылает во фронтенд
        # (sidecar://progress) для прогресс-индикатора.
        _say(json.dumps({'event': 'SIDECAR_BOOT', 'stage': stage}, ensure_ascii=False))
...
if __name__ == '__main__':
    # v3.5: тяжёлые импорты (cv2/openpyxl/PIL) завершены — стадия 1/2.
    # Вызывается до main(): main() делает только конфигурацию (мгновенно).
    _say(json.dumps({'event': 'SIDECAR_BOOT', 'stage': 'imports'}, ensure_ascii=False))
    main()
