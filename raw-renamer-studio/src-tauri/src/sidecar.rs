...
/// v3.5: стадия запуска sidecar для прогресс-индикатора UI.
/// 0 = процесс запущен, 1 = импорты завершены, 2 = сервер принимает, 3 = READY.
#[derive(Serialize, Clone)]
    ...
    for line in reader.lines() {
        ...
        if !ready_sent {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&line) {
                ...
                // v3.5: стадии запуска (imports / listen) → прогресс-индикатор.
                if event == Some("SIDECAR_BOOT") {
                    let stage: u8 = match v.get("stage").and_then(|s| s.as_str()) {
                        Some("imports") => 1,
                        Some("listen") => 2,
                        _ => 0,
                    };
                    let _ = app.emit("sidecar://progress", ProgressPayload { stage });
                }
            }
        }
...
fn supervisor_loop<R: Runtime>(
    ...
    loop {
        ...
        *pid.lock().unwrap_or_else(|e| e.into_inner()) = Some(id);
        ...
        // v3.5: прогресс — стадия 0 (процесс запущен, распаковка/импорты).
        let _ = app.emit("sidecar://progress", ProgressPayload { stage: 0 });
