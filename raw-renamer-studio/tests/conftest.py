...
class Sidecar:
    def __init__(self, tmp: Path, watchdog: float = 0.0):
        self.tmp = tmp
...
class TestBatch:
    """Тестовая партия (RAW + PIM + заявка), сгенерированная в tmp.

    v3.5: тестовые данные не лежат в репо — фикстура сама генерирует партию через tools/make_test_batch.py (session-scoped,
    один раз на запуск pytest).
    """

    def __init__(self, root: Path):
        self.root = root
        self.raw_folder = root / 'shoot_2026-09-16'
        self.zayavka = root / 'zayavka_17.09.2026.xlsx'
        self.pim = root / 'export_pim.csv'
        assert self.raw_folder.is_dir(), 'генерация тестовой партии: нет shoot-папки'
        assert self.zayavka.is_file(), 'генерация тестовой партии: нет заявки'


@pytest.fixture(scope='session')
def batch(tmp_path_factory) -> TestBatch:
    bdir = tmp_path_factory.mktemp('test_batch')
    subprocess.run(
        [sys.executable, str(HERE / 'tools' / 'make_test_batch.py'), str(bdir)],
        check=True, capture_output=True, text=True,
    )
    return TestBatch(bdir)
